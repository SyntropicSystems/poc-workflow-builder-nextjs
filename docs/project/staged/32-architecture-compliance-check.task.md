...
Verify that the implementation follows the intended architecture patterns and that Protobuf is being used correctly as the source of truth for all types.

## Rationale

The architecture must follow these principles for Rust migration:

- Protobuf schema is the single source of truth
- Generated types are used everywhere (never handwritten)
- API functions use Result<T> pattern (Rust-like)
- Core logic has no framework dependencies

## Analysis Steps

### Step 1: Create Architecture Compliance Checker

Create `scripts/check-architecture.py`:
...
```bash
chmod +x scripts/check-architecture.py
...
1. `reports/architecture-compliance.json` - Detailed findings
1. `reports/architecture-compliance.md` - Summary report

## Success Criteria

- [ ] Protobuf schema exists and is complete
- [ ] Generated types are used everywhere
- [ ] No handwritten Flow/Step/Policy types
- [ ] API uses Result<T> pattern consistently
- [ ] Core module has zero framework dependencies
- [ ] Clean layer separation maintained
- [ ] All imports follow dependency rules

## Architecture Rules Being Checked

### Layer Dependencies

```
app/ → components/ → lib/state/ → lib/workflow-core/
         ↓              ↓              ↓
...
1. **Only generated types**: No manual interface Flow/Step/Policy
1. **Import from generated**: All types from `lib/workflow-core/generated`
1. **API wraps types**: Components import types through API, not directly

### Core Purity Rules

1. **No React**: No hooks, components, or React imports
1. **No Next.js**: No Next.js specific imports
1. **No DOM**: No window, document, or browser APIs
1. **Pure Functions**: All functions are pure with no side effects

## Red Flags

1. **Manual types**: `interface Flow {` in non-generated files
1. **Framework in core**: `import React` in workflow-core
1. **Wrong imports**: Components importing from `/generated` directly
1. **Layer violations**: API importing from components
1. **Missing Result**: API functions returning raw values not Result<T>